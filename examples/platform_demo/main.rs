...
fn fps_controller(
	...
	let stick_rx = apply_deadzone(
		input
			.gamepad(0)
			.map_or(0.0, |gp| gp.axis(GamepadAxis::RightStickX)),
	);
	let stick_ry = apply_deadzone(
		input
			.gamepad(0)
			.map_or(0.0, |gp| gp.axis(GamepadAxis::RightStickY)),
	);
	...
	*pitch = (*pitch + dy * SENSITIVITY + stick_ry * STICK_LOOK_SPEED * dt)
	...
	let stick_mx = apply_deadzone(
		input
			.gamepad(0)
			.map_or(0.0, |gp| gp.axis(GamepadAxis::LeftStickX)),
	);
	let stick_my = apply_deadzone(
		input
			.gamepad(0)
			.map_or(0.0, |gp| gp.axis(GamepadAxis::LeftStickY)),
	);
...
fn fov_controller(
	...
	if input.is_key_just_pressed(KeyCode::Equals) {
		delta += FOV_STEP;
	}
...
fn render_scale_controller(input: Res<InputState>, mut quality: ResMut<QualitySettings>) {
	let mut delta = 0.0f32;
	if input.is_key_just_pressed(KeyCode::LeftBracket) {
		delta -= 0.1;
	}
	if input.is_key_just_pressed(KeyCode::RightBracket) {
		delta += 0.1;
	}
	if delta == 0.0 {
		return;
...
fn quit_on_escape(input: Res<InputState>) {
	...
	let controller_quit = input.gamepad(0).is_some_and(|gp| {
		gp.is_button_just_pressed(GamepadButton::Start)
