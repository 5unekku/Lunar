...
fn setup(mut commands: Commands, mut assets: ResMut<AssetServer>) {
	...
	commands.spawn((Player, Transform::from_xy(ARENA_W * 0.5, ARENA_H - 60.0)));
}
...
fn player_input(
	...
	transform.translation.x =
		(transform.translation.x + dx).clamp(PLAYER_W * 0.5, ARENA_W - PLAYER_W * 0.5);
	...
	if fire_timer.0 <= 0.0 && input.is_key_held(KeyCode::Space) {
		...
		commands.spawn((
			...
			Transform::from_xy(
				transform.translation.x,
				transform.translation.y - PLAYER_H * 0.5,
			),
		));
	}
}
...
fn enemy_spawner(
	...
	let base_x =
		pseudo_rand_f32(time.elapsed_seconds()) * (ARENA_W - ENEMY_W * 4.0) + ENEMY_W * 2.0;
	for i in 0..3 {
		let x = base_x + i as f32 * (ENEMY_W + 10.0);
		if x + ENEMY_W * 0.5 > ARENA_W {
			break;
		}
		commands.spawn((Enemy, Transform::from_xy(x, -ENEMY_H)));
	}
}
