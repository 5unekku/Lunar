/// pixel-space scissor rectangle used for per-light and per-interaction culling.
///
/// the render system computes one of these for every light by projecting its
/// bounding volume onto the screen. only the pixels inside the rect are shaded
/// for that light, which dramatically reduces fillrate when many small lights
/// are present.
///
/// i16 pixel coordinates keep the struct at 12 bytes — tight enough to cache
/// one per light without pressure. z bounds are f32 for depth-bounds testing
/// on hardware that supports it (reduces overdraw further).
///
