//! kinematic character controller for 3D — move, slide, handle slopes and steps.
//!
//! not a full physics simulation — designed for character movement. attach
//! [`KinematicBody3d`] and set velocity each frame; the physics system resolves
//! collisions, grounds the character, and applies gravity automatically.
//!
//! features: slope detection, automatic step-up, one-shot gravity, grounded state.
//!
//! # usage
//!
//! ```ignore
...
pub fn move_and_slide_3d(
	...
	for (entity, mut transform, mut body, collider) in query.iter_mut() {
		...
		for _ in 0..body.solver_iterations {
			...
			for other in
				collision_world.overlapping_at(entity, new_pos, half_extents, collider.mask)
			{
				let overlap =
					aabb_overlap_depth(new_pos, half_extents, other.position, other.half_extents);
...
impl CollisionWorldExt for CollisionWorld3d {
	fn overlapping_at(
		...
		self.query_aabb_entries(qmin_x, qmax_x)
...
fn aabb_overlap_depth(pos_a: Vec3, half_a: Vec3, pos_b: Vec3, half_b: Vec3) -> Option<(Vec3, f32)> {
...
mod tests {
	...
	fn body_resolves_floor_collision() {
		...
		assert!(
			body.is_grounded,
			"character should be grounded after floor collision"
		);
		assert!(
			body.velocity.y >= 0.0,
			"downward velocity should be zeroed on ground contact"
		);
	}
	...
	fn no_overlap_returns_none() {
		let result = aabb_overlap_depth(Vec3::new(5.0, 0.0, 0.0), Vec3::ONE, Vec3::ZERO, Vec3::ONE);
		assert!(result.is_none());
	}
}

/// common, game-facing 3D physics types for `use lunar::prelude::*`.
pub mod prelude {
	pub use crate::{Gravity3d, KinematicBody3d, PhysicsPlugin3d};
}
