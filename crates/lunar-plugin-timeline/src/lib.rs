...
//!
//! a [`Timeline`] component holds a list of [`TimelineTrack`]s. each track is a
//! sorted list of [`TimelineKey`]s — (timestamp, action) pairs. `tick_timelines`
//! fires actions whose timestamps the playhead crosses each frame.
//!
//! # usage
//!
//! ```ignore
...
///
/// tick with [`tick_timelines`] every frame.
#[derive(Component)]
pub struct Timeline {
        ...
        for (target, action) in pending {
            match action {
                ...
                TimelineAction::FireEvent(name) => {
                    timeline_events.pending.push(TimelineEvent { timeline_entity, target, name });
                }
            }
        }
...
mod tests {
    ...
    fn set_delta(world: &mut World, delta: f32) {
        world.resource_mut::<lunar_core::Time>().set_delta_seconds(delta);
    }
    ...
    fn move_to_action_updates_transform() {
        ...
        let mut timeline = Timeline::new(vec![TimelineTrack::new(target, vec![TimelineKey {
            time: 0.1,
        ...
        let pos = world.get::<lunar_math::LocalTransform>(target).unwrap().translation;
    ...
    fn looping_timeline_wraps_playhead() {
        ...
        assert_eq!(world.get::<Timeline>(entity).unwrap().state, TimelineState::Playing);
    }
}

/// drop-in plugin: inserts [`TimelineEvents`] and runs [`tick_timelines`] in
/// `Update`. drain `TimelineEvents` each frame to handle custom timeline events.
#[derive(Default)]
