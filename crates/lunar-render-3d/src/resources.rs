...
impl RenderEngine3d {
	pub(crate) fn build_render_graph(
		...
		if ssr {
			g.add_pass("ssr", vec![hdr, depth], vec![ssr_tex]);
		}
		if fog {
			g.add_pass("volumetric_fog", vec![depth], vec![fog_tex]);
		}
		if bloom {
			g.add_pass("bloom", vec![hdr], vec![bloom_tex]);
		}
		let composite_reads = {
			let mut r = vec![hdr];
			if ssao {
	...
	pub(crate) fn append_to_mega_buffers(&mut self, mesh_id: u32, data: &MeshData) {
		...
		if self.mega_vbuf_bytes + vertex_bytes > self.mega_vbuf.as_ref().unwrap().size() {
			let new_size = (self.mega_vbuf.as_ref().unwrap().size() * 2)
				.max(self.mega_vbuf_bytes + vertex_bytes);
		...
		if self.mega_ibuf_bytes + index_bytes > self.mega_ibuf.as_ref().unwrap().size() {
			let new_size = (self.mega_ibuf.as_ref().unwrap().size() * 2)
				.max(self.mega_ibuf_bytes + index_bytes);
			let new_buf = self.device.create_buffer(&wgpu::BufferDescriptor {
				label: Some("[mega] ibuf"),
				size: new_size,
				usage: wgpu::BufferUsages::INDEX | wgpu::BufferUsages::COPY_DST,
		...
		let idx32: Vec<u32> = match &data.indices {
			...
			IndexBuffer::U16(v) => {
				use rayon::prelude::*;
				v.par_iter().map(|&x| x as u32).collect()
			}
			#[cfg(target_arch = "wasm32")]
	...
	pub(crate) fn ensure_hzb_resources(&mut self) {
		...
		self.hzb_mip_views = (0..mip_count)
			.map(|mip| {
				hzb_tex.create_view(&wgpu::TextureViewDescriptor {
		...
		self.hzb_depth_src_view =
			Some(depth_src.create_view(&wgpu::TextureViewDescriptor::default()));
		...
		self.hzb_copy_pipeline = Some(self.device.create_compute_pipeline(
			&wgpu::ComputePipelineDescriptor {
		...
		self.hzb_downsample_pipeline = Some(self.device.create_compute_pipeline(
			&wgpu::ComputePipelineDescriptor {
		...
		self.hzb_cull_pipeline = Some(self.device.create_compute_pipeline(
			&wgpu::ComputePipelineDescriptor {
	...
	pub(crate) fn ensure_contact_shadow_resources(
		...
		if self.contact_shadow_bgl.is_none() {
			...
			let params_buf = self.device.create_buffer(&wgpu::BufferDescriptor {
				...
				size: CONTACT_SHADOW_PARAMS_SIZE,
				usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
				mapped_at_creation: false,
			});
	...
	pub(crate) fn ensure_motion_vector_resources(&mut self, width: u32, height: u32) {
		...
		if self.motion_vec_bgl.is_none() {
			...
			let params_buf = self.device.create_buffer(&wgpu::BufferDescriptor {
				label: Some("[motion vec] params"),
				size: MOTION_VECTOR_PARAMS_SIZE,
				usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
				mapped_at_creation: false,
			});
	...
	pub(crate) fn ensure_detail_sprite_resources(&mut self) {
		if self.detail_sprite_bgl.is_some() {
			return;
		}
				...
				entries: &[
					wgpu::BindGroupEntry {
						...
						resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
							buffer: params_buf,
							offset: (i * UNIFORM_STRIDE as usize) as u64,
							size: wgpu::BufferSize::new(BLOOM_PARAMS_SIZE),
						}),
					},
					wgpu::BindGroupEntry {
						binding: 1,
			...
			us_bgs.push(device.create_bind_group(&wgpu::BindGroupDescriptor {
				...
				entries: &[
					wgpu::BindGroupEntry {
						...
						resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
							buffer: params_buf,
							offset: ((actual_mips + i) * UNIFORM_STRIDE as usize) as u64,
							size: wgpu::BufferSize::new(BLOOM_PARAMS_SIZE),
						}),
					},
					wgpu::BindGroupEntry {
	...
	pub(crate) fn ensure_upscale_resources(
		&mut self,
		render_w: u32,
		render_h: u32,
		display_w: u32,
		...
		let needs_tex_rebuild = self
			.fsr_ldr_texture
			.as_ref()
			.map(|t| {
				let s = t.size();
				s.width != render_w || s.height != render_h
			})
			.unwrap_or(true)
			|| self
				.fsr_mid_texture
				...
				.map(|t| {
					let s = t.size();
					s.width != display_w || s.height != display_h
				})
				.unwrap_or(true);
					...
					entries: &[
						wgpu::BindGroupLayoutEntry {
							...
							ty: wgpu::BindingType::Buffer {
								ty: wgpu::BufferBindingType::Uniform,
								has_dynamic_offset: false,
								min_binding_size: wgpu::BufferSize::new(FSR_PARAMS_SIZE),
							},
							count: None,
						},
						wgpu::BindGroupLayoutEntry {
		...
		if needs_tex_rebuild {
			...
			self.fsr_easu_bg = Some(make_bg(
				&self.device,
				bgl,
				params_buf,
				ldr_view,
				&self.post_sampler,
				"[upscale] easu/single bg",
			));
			self.fsr_rcas_bg = Some(make_bg(
				&self.device,
				bgl,
				params_buf,
				mid_view,
