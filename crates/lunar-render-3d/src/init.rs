...
impl RenderEngine3d {
	...
	pub(crate) fn init_with_adapter(
		...
		Self {
			...
			terrain_snap_scratch: Vec::new(),
			water_scratch: Vec::new(),
			decal_scratch: Vec::new(),
			detail_scratch: Vec::new(),
			point_shadow_layer_scratch: Vec::new(),
			queries: None,
			...
			// LUNAR_NO_GPU_CULL / LUNAR_NO_HZB force the CPU-only path for a/b bisection
			// of cull-related visual bugs. the fresh CPU frustum sweep runs regardless.
			gpu_cull_enabled: render_tier == RenderTier::High
				&& std::env::var_os("LUNAR_NO_GPU_CULL").is_none(),
			cull_aabb_buf: None,
			cull_flags_buf: None,
			...
			hzb_enabled: render_tier == RenderTier::High
				&& std::env::var_os("LUNAR_NO_HZB").is_none(),
			hzb_texture: None,
