...
impl RenderEngine3d {
	...
	pub(crate) fn record_post_processing(
		&mut self,
		fc: &FrameContext,
		world: &World,
		...
		if let Some(atmos) = world.get_resource::<AtmosphericScattering>().copied() {
			...
			self.queue
				.write_buffer(&self.atmos_params_buf, 0, &atmos_data);
		...
		if self.fog_enabled && dev_fog {
			...
			let rest: [f32; 16] = [
				...
				0.6_f32,
				width,
				height, // sun_intensity, anisotropy
			];
		...
		if dev_contact_shadows {
			...
			if let Some(params_buf) = self.contact_shadow_params_buf.as_ref() {
				let light_dir_data: [f32; 3] =
					[light_dir_vs_raw.x, light_dir_vs_raw.y, light_dir_vs_raw.z];
				self.queue
					...
					entries: &[
						...
						wgpu::BindGroupEntry {
							...
							resource: wgpu::BindingResource::TextureView(
								self.bloom_mip_views.first().unwrap_or(&self.hdr_view),
							),
						},
						wgpu::BindGroupEntry {
							binding: 3,
		...
		// ── composite pass → swapchain ────────────────────────────────────
		{
			...
			let (
				...
				let grain_s;
				if let Some(q) = q {
					if self.bloom_enabled && dev_bloom && q.bloom {
						f |= 1;
					}
					if dev_vignette && q.vignette {
						f |= 2;
					}
					if dev_chrom_ab && q.chromatic_aberration {
						f |= 4;
					}
					if dev_film_grain && q.film_grain {
						f |= 8;
					}
					if self.ssao_enabled && dev_ssao && q.ssao {
						f |= 16;
					}
					if dev_ssr {
						f |= 32;
					...
					vig_r = 0.3_f32;
					ca_s = if dev_chrom_ab && q.chromatic_aberration {
						1.5
					} else {
						0.0
					};
					grain_s = if dev_film_grain && q.film_grain {
						0.5
					} else {
						0.0
					};
					...
					ca_s = 0.0;
					grain_s = 0.0;
					if self.bloom_enabled && dev_bloom {
						f |= 1;
					}
					if dev_contact_shadows && self.contact_shadow_tex.is_some() {
						f |= 128;
		...
		if self.upscale_active
			...
			let upscale_data: [f32; 8] = [
				self.render_w as f32,
				self.render_h as f32,
				dw,
				dh,
				0.25,
				0.0,
				0.0,
				0.0, // rcas_sharpness + padding
			];
			self.queue
				.write_buffer(params_buf, 0, bytemuck::cast_slice(&upscale_data));
			...
			match upscale_mode {
				UpscaleMode::Nearest => {
					if let Some(pl) = &self.upscale_nearest_pipeline {
						run_pass(
							&mut *encoder,
							"[upscale] nearest",
							pl,
				...
				UpscaleMode::Linear => {
					if let Some(pl) = &self.upscale_linear_pipeline {
						run_pass(&mut *encoder, "[upscale] linear", pl, easu_bg, final_target);
					}
				}
				UpscaleMode::Lanczos => {
					if let Some(pl) = &self.upscale_lanczos_pipeline {
						run_pass(
							&mut *encoder,
							"[upscale] lanczos",
							pl,
				...
				UpscaleMode::Bicubic => {
					if let Some(pl) = &self.upscale_bicubic_pipeline {
						run_pass(
							&mut *encoder,
							"[upscale] bicubic",
							pl,
		...
		if dev_staa {
			...
			taa_data[0..64].copy_from_slice(bytemuck::cast_slice(
			...
			taa_data[152..156]
				.copy_from_slice(bytemuck::cast_slice(&[self.render_w as f32 / w as f32]));
			taa_data[156..160]
				.copy_from_slice(bytemuck::cast_slice(&[self.render_h as f32 / h as f32]));
			...
			let history_write_view = if write_b {
				&self.staa_history_b_view
			} else {
				&self.staa_history_a_view
			};
		...
		if dev_fxaa && !dev_staa {
			...
			self.queue
				.write_buffer(&self.fxaa_params_buf, 0, bytemuck::cast_slice(&fxaa_data));
	...
	pub(crate) fn record_gtao_reflection(
		&mut self,
		fc: &FrameContext,
		world: &mut World,
		encoder: &mut wgpu::CommandEncoder,
		...
		if gtao_active || sky_needs_depth {
			...
			let gtao_params: [f32; 40] = {
				...
				d[34] = far;
				d[35] = if self.render_tier == RenderTier::High {
					5.0
				} else {
					3.0
				}; // slice_count
				d[36] = if self.render_tier == RenderTier::High {
					6.0
				} else {
					4.0
				}; // step_count
			...
			self.queue
				.write_buffer(&self.gtao_params_buf, 0, bytemuck::cast_slice(&gtao_params));
			...
			let mut reflector: Option<(f32, u32)> = None; // (plane_y, resolution_divisor)
			{
				...
				for (refl, wt, vis) in rq.iter(world) {
					...
					if reflector.is_none() {
						reflector = Some((
							refl.plane_y + wt.translation.y,
							refl.resolution_divisor.max(1),
						));
					}
				}
			}
			if let Some((plane_y, div)) = reflector {
				...
				if needs_new {
					...
					self.reflection_view =
						Some(rt.create_view(&wgpu::TextureViewDescriptor::default()));
				...
				let refl_cam_pos =
					lunar_math::Vec3::new(cam_pos.x, 2.0 * plane_y - cam_pos.y, cam_pos.z);
				...
				if depth_needs_new {
					...
					self.reflection_depth_view =
						Some(dt.create_view(&wgpu::TextureViewDescriptor::default()));
