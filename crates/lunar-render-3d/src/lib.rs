...
impl Default for AutoQuality {
	fn default() -> Self {
		Self {
			enabled: false,
			min: QualityPreset::Minimum,
			max: QualityPreset::Ultra,
		}
	}
}
...
impl DevRenderProfile {
	...
	#[must_use]
	pub fn with_point_light_shadows(mut self, enabled: bool) -> Self {
		self.point_light_shadows = enabled;
		self
	}
	#[must_use]
	pub fn with_max_point_lights(mut self, count: u32) -> Self {
		self.max_point_lights = count;
		self
	}
	#[must_use]
	pub fn with_soft_shadows(mut self, enabled: bool) -> Self {
		self.soft_shadows = enabled;
		self
	}
	#[must_use]
	...
	#[must_use]
	pub fn with_volumetric_fog(mut self, enabled: bool) -> Self {
		self.volumetric_fog = enabled;
		self
	}
	#[must_use]
	pub fn with_fxaa(mut self, enabled: bool) -> Self {
		self.fxaa = enabled;
		self
	}
	#[must_use]
	...
	#[must_use]
	pub fn with_vignette(mut self, enabled: bool) -> Self {
		self.vignette = enabled;
		self
	}
	#[must_use]
	pub fn with_chromatic_aberration(mut self, enabled: bool) -> Self {
	...
	#[must_use]
	pub fn with_max_shadow_cascades(mut self, count: u32) -> Self {
		self.max_shadow_cascades = count;
		self
	}
	#[must_use]
	pub fn with_max_msaa(mut self, samples: u32) -> Self {
		self.max_msaa = samples;
		self
	}
	#[must_use]
...
///
/// passed to `bootstrap_3d` at startup. the render engine
...
fn render_3d_system(world: &mut World) {
	...
	{
		let (over_f, under_f) = (
			engine.auto_quality_over_frames,
			engine.auto_quality_under_frames,
		);
		let current = world.resource::<QualitySettings>().preset;
		let tier = engine.tier();
		if over_f >= OVER_THRESHOLD
			&& RenderEngine3d::preset_ord(current) > RenderEngine3d::preset_ord(auto.min)
		{
		...
		} else if under_f >= UNDER_THRESHOLD
			&& RenderEngine3d::preset_ord(current) < RenderEngine3d::preset_ord(auto.max)
		{
