...
impl RenderEngine3d {
	...
	pub(crate) fn record_scene_passes(
		&mut self,
		fc: &FrameContext,
		world: &mut World,
			...
			if self.gpu_indirect_active() {
				...
				while i <= n {
					...
					if group_changed && i > group_start {
						let Some(gpu_mesh) = self.mesh_gpu.get(&last_mesh) else {
							group_start = i;
							i += 1;
							continue;
						};
					...
					if cur_mesh != last_mesh
						...
						|| cur_dir_lm != last_dir_lm
					{
						let Some(gpu_mesh) = self.mesh_gpu.get(&cur_mesh) else {
							i += 1;
							continue;
						};
						let lm_bg = if cur_lm != u32::MAX {
		...
		// ── terrain pass — geometry clipmap heightmap rendering ─────────
		{
			...
			for snap in &terrain_snaps {
				...
				let needs_rebuild = !self.terrain_gpu.contains_key(&entity) || snap.dirty;
				if needs_rebuild {
				...
				let Some(gpu) = self.terrain_gpu.get(&entity) else {
					continue;
				};
				...
				let effective_rings = if self.render_tier == RenderTier::LowGles {
					1
				} else {
					rings
				};
				...
				for ring in 0..effective_rings as usize {
					...
					let ring_origin_x =
						(cam_pos.x / lod_cell_size).floor() * lod_cell_size - ring_half;
					let ring_origin_z =
		...
		if self.render_tier != RenderTier::LowGles {
			...
			for (water_comp, mesh_id, wt) in &water_entities {
				...
				let water_color = [
					water_comp.water_color.r,
					water_comp.water_color.g,
					water_comp.water_color.b,
					water_comp.water_color.a,
				];
				let deep_color = [
					water_comp.deep_color.r,
					water_comp.deep_color.g,
					water_comp.deep_color.b,
					water_comp.deep_color.a,
				];
		...
		if self.particles_enabled {
			...
			for (emitter, wt) in emitter_query.iter(world) {
				...
				for n in 0..new_count {
					...
					self.particle_spawn_scratch.push(CpuParticle {
						...
						color_start: [
							emitter.color_start.r,
							emitter.color_start.g,
							emitter.color_start.b,
							emitter.color_start.a,
						],
						color_end: [
							emitter.color_end.r,
							emitter.color_end.g,
			...
			for slot in 0..self.particle_cpu.len() {
				if spawn_idx >= self.particle_spawn_scratch.len() {
					break;
				}
				if self.particle_cpu[slot].alive {
					continue;
				}
				let spawned = self.particle_spawn_scratch[spawn_idx];
				spawn_idx += 1;
				self.particle_gpu_writes
					.push((slot as u32, spawned.as_gpu()));
			...
			while i < self.particle_gpu_writes.len() {
				...
				self.queue.write_buffer(
					&self.particle_buf,
					start_slot as u64 * PARTICLE_STRIDE,
			...
			if !detail_entities.is_empty() {
				...
				for (entity, dd, _base_y) in &detail_entities {
					...
					if !self.detail_sprite_cache.contains_key(&entity_key) {
						...
						self.detail_sprite_cache.insert(
							entity_key,
							DetailSpriteEntry {
								inst_buf,
								count_buf,
						...
						if entry.density_key != density_key {
							let (params_buf, inst_buf, count_buf) = (
								entry.params_buf.clone(),
								entry.inst_buf.clone(),
								entry.count_buf.clone(),
							);
							if let Some(bg) = self.build_detail_compute_bg(
								&params_buf,
								&inst_buf,
								&count_buf,
								density_id,
					...
					self.queue
						.write_buffer(&entry.count_buf, 0, &0u32.to_le_bytes());
					...
					self.queue
						.write_buffer(&entry.draw_buf, 0, bytemuck::cast_slice(&draw_init));
					...
					self.queue.write_buffer(
						&entry.params_buf,
						48,
						bytemuck::cast_slice(&compute_params_tail),
					);
					...
					if let Some(compute_pipeline) = self.detail_sprite_compute_pipeline.as_ref() {
						...
						cpass.dispatch_workgroups(
							grid_count_x.div_ceil(8),
							grid_count_z.div_ceil(8),
							1,
						);
					}
		...
		if dev_point_shadows {
			...
			for &(light_pos, _, _, light_radius, casts, _) in self.point_light_scratch.iter() {
				...
				for face in 0..6usize {
					if !self.point_shadow_dirty[pt_shadow_idx][face] {
						continue;
					}
						...
						if task == usize::MAX {
							...
							while i <= n {
								...
								if (cur_mesh != last_mesh || cur_mat != last_mat)
									&& i > group_start && let Some(gpu) = s_mesh_gpu.get(&last_mesh)
								{
									let base = (ENTITY_SLOT_START + group_start) as u32;
									zpass.draw_indexed(
										0..gpu.index_count,
