...
impl std::fmt::Display for PersistError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PersistError::NotSupported => formatter.write_str("persist: not supported on this platform"),
...
mod tests {
    ...
    fn round_trip_basic() {
        ...
        let original = TestData { level: 5, score: 9999, name: "hero".into() };
    ...
    fn round_trip_nested_dir() {
        let tmp = std::env::temp_dir().join("lunar_persist_nested").join("slot0.ron");
        let path = tmp.to_str().unwrap();
        let original = TestData { level: 1, score: 0, name: "new game".into() };
