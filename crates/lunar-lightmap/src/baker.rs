...
impl LightmapBaker {
	...
	#[must_use]
	pub fn with_dilation(mut self, texels: u32) -> Self {
		self.dilation = texels;
		self
	}
	...
	pub fn bake(&self, mesh: &MeshData) -> BakeResult {
		...
		let row_data: Vec<Vec<u8>> = (0..h)
			.into_par_iter()
			.map(|row| {
				let mut row_pixels = vec![0u8; w as usize * 4];
				for col in 0..w {
					let uv =
						Vec2::new((col as f32 + 0.5) / w as f32, (row as f32 + 0.5) / h as f32);
	...
	pub fn bake_directional(&self, mesh: &MeshData) -> (BakeResult, BakeResult) {
		...
		let row_data: Vec<(Vec<u8>, Vec<u8>)> = (0..h)
			.into_par_iter()
			.map(|row| {
				...
				for col in 0..w {
					...
					if let Some(surface) = find_surface(&tris, uv) {
						...
						dir_row[idx + 1] =
							((dominant_dir.y * 0.5 + 0.5).clamp(0.0, 1.0) * 255.0) as u8;
						dir_row[idx + 2] =
							((dominant_dir.z * 0.5 + 0.5).clamp(0.0, 1.0) * 255.0) as u8;
						dir_row[idx + 3] = 255;
					}
				}
				(irr_row, dir_row)
		...
		let mut irr = BakeResult {
			width: w,
			height: h,
			pixels: irr_pixels,
		};
		let mut dir = BakeResult {
			width: w,
			height: h,
			pixels: dir_pixels,
		};
