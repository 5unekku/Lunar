...
pub fn extract_portals(
    ...
    let mut portals: Vec<PortalData> = hints.iter().map(|h| PortalData {
        area_a: h.area_a,
        area_b: h.area_b,
    ...
    for a in 0..leaf_count {
        let area_a = match leaf_areas[a] { Some(id) => id, None => continue };
        for b in (a + 1)..leaf_count {
            let area_b = match leaf_areas[b] { Some(id) => id, None => continue };
            if area_a == area_b { continue; }

            if let Some(portal) = aabb_adjacency_portal(
                leaf_aabbs[a], leaf_aabbs[b], area_a, area_b
            ) {
                ...
                if !already { portals.push(portal); }
            }
        }
    }
    portals
...
fn aabb_adjacency_portal(
    ...
    for axis in 0usize..3 {
        ...
        let touching = (amax[axis] - bmin[axis]).abs() < EPS
                    || (bmax[axis] - amin[axis]).abs() < EPS;
        ...
        return Some(PortalData { area_a, area_b, center, half_extents });
    }
    None
}
