...
pub fn build_bsp(triangles: &[InputTriangle], max_leaf_size: usize) -> PartitionResult {
    ...
    if triangles.is_empty() { return result; }
...
fn tri_aabb(tri: &InputTriangle) -> ([f32; 3], [f32; 3]) {
    ...
    for v in &tri.verts {
        ...
        for a in 0..3 {
            if coords[a] < min[a] { min[a] = coords[a]; }
            if coords[a] > max[a] { max[a] = coords[a]; }
        }
    }
    (min, max)
}
...
fn build_node(
    ...
    for &i in indices {
        ...
        for a in 0..3 {
            if tmin[a] < min[a] { min[a] = tmin[a]; }
            if tmax[a] > max[a] { max[a] = tmax[a]; }
        }
    }
        ...
        for bucket in 1..BUCKETS {
            ...
            for &i in indices {
                ...
                if c_arr[axis as usize] < split {
                    lcount += 1;
                    for a in 0..3 {
                        if tmin[a] < lmin[a] { lmin[a] = tmin[a]; }
                        if tmax[a] > lmax[a] { lmax[a] = tmax[a]; }
                    }
                } else {
                    rcount += 1;
                    for a in 0..3 {
                        if tmin[a] < rmin[a] { rmin[a] = tmin[a]; }
                        if tmax[a] > rmax[a] { rmax[a] = tmax[a]; }
                    }
                }
            }
    ...
    if best_cost.is_infinite() {
        ...
        best_axis = if extents[0] >= extents[1] && extents[0] >= extents[2] { 0 }
                    else if extents[1] >= extents[2] { 1 } else { 2 };
        best_split = (min[best_axis as usize] + max[best_axis as usize]) * 0.5;
    }

    let (left_indices, right_indices): (Vec<usize>, Vec<usize>) = indices.iter().copied().partition(|&i| {
        let c = tri_centroid(&triangles[i]);
        let c_arr = [c.x, c.y, c.z];
...
fn dominant_area(triangles: &[InputTriangle], indices: &[usize]) -> Option<u32> {
    ...
    counts.into_iter().max_by_key(|(_, count)| *count).map(|(area, _)| area)
}
