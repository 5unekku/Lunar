...
//!
//! ```ignore
//! use lunar::spline::{Spline, PathFollower, SplinePlugin, SplineStore};
//! use lunar::Vec3;
//!
...
///
/// pair with `LocalTransform` (2D) or `LocalTransform3d` (3D).
/// add [`SplinePlugin`] to animate all path followers automatically each frame.
#[derive(Component)]
...
/// resource — stores named splines that [`PathFollower`] components reference.
///
...
mod tests {
	...
	fn tangent_nonzero_on_curve() {
		...
		assert!(
			tangent.length() > 0.01,
			"tangent on a straight spline should be nonzero"
		);
...
/// drop-in plugin: inserts an empty [`SplineStore`] and ticks path followers each frame.
/// add splines to the store in a setup system, then attach [`PathFollower`] components.
#[derive(Default)]
pub struct SplinePlugin;

impl lunar_core::GamePlugin for SplinePlugin {
	fn name(&self) -> &str {
		"SplinePlugin"
	}
	fn build(&mut self, app: &mut lunar_core::App) {
		app.insert_resource(SplineStore::default());
		app.add_system_to_stage(
			lunar_core::UpdateStage::Update,
			(advance_path_followers, advance_path_followers_3d),
		);
	}
}

/// common, game-facing spline types for `use lunar::prelude::*`.
pub mod prelude {
	pub use crate::{PathFollower, Spline, SplinePlugin, SplineStore};
}
