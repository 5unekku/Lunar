...
/// drop-in plugin: inserts an empty [`SplineStore`] and runs the path-follower
/// advance systems (2D + 3D) in `Update`. add your splines to the store, then
/// attach [`PathFollower`] components. raw systems stay `pub` for hand-ordering.
#[derive(Default)]
