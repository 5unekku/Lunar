...
mod tests {
	...
	fn arm_direction_yaw_zero_points_positive_z() {
		...
		assert!(
			dir.z > 0.0,
			"arm direction should have +Z component with zero yaw"
		);
	}
...
/// drop-in plugin: runs [`spring_arm_system`] in `PostUpdate` (after transforms
/// propagate). attach a [`SpringArm3d`] to your camera entity, pointing at a target.
#[derive(Default)]
pub struct SpringArmPlugin;

impl lunar_core::GamePlugin for SpringArmPlugin {
	fn name(&self) -> &str {
		"SpringArmPlugin"
	}
	fn build(&mut self, app: &mut lunar_core::App) {
		app.add_system_to_stage(lunar_core::UpdateStage::PostUpdate, spring_arm_system);
	}
}

/// common, game-facing camera types for `use lunar::prelude::*`.
pub mod prelude {
	pub use crate::{SpringArm3d, SpringArmPlugin};
}
