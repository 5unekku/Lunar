...
//!
//! drives [`Sprite`] frame selection from named clips defined as sequences of
//! spritesheet regions and durations. supports looping, one-shot, and manual control.
//!
//! # usage
//!
//! ```ignore
...
/// call [`Animator::play`] to switch clips. the animation system updates the
/// sprite's visible frame each tick based on the current clip and elapsed time.
...
pub fn advance_animations(
	...
	for (entity, mut animator, mut sprite) in &mut query {
		...
		let Some(clip_idx) = animator.current_clip_idx else {
			continue;
		};
		let clip_name_str = &animator.clip_names[clip_idx];
		let Some(clip) = animator.clips.get(clip_name_str.as_str()) else {
			continue;
		};
		...
		let clip = animator
			.clips
			.get(animator.clip_names[clip_idx].as_str())
			.unwrap();
		...
		let frame_rect = (
			clip.frames[new_index].source_pos,
			clip.frames[new_index].source_size,
		);
...
///
/// add this alongside `RenderPlugin` to enable frame-by-frame sprite animation.
...
/// common, game-facing animation types for `use lunar::prelude::*`.
pub mod prelude {
	pub use crate::{AnimationClip, AnimationFinished, AnimationFrame, AnimationPlugin, Animator};
}
