...
impl<T: Asset> AssetStore<T> {
	...
	fn loaded_count(&self) -> usize {
		self.entries
			.iter()
			.flatten()
			.filter(|e| e.state == LoadState::Loaded)
			.count()
	}
	...
	fn failed_count(&self) -> usize {
		self.entries
			.iter()
			.flatten()
			.filter(|e| e.state == LoadState::Failed)
			.count()
	}
...
impl TextureLoaderTrait for BctexLoader {
	fn load(&self, bytes: Vec<u8>) -> Result<Texture, String> {
		if bytes.len() < 16 {
			return Err("bctex: too short".into());
		}
		if &bytes[0..4] != b"BCTX" {
			return Err("bctex: bad magic".into());
		}
...
impl AssetServer {
	...
	pub fn desired_mip_count(&self, tex_id: u32, max_mip_levels: u32) -> u32 {
		...
		if coverage >= 0.5 || max_mip_levels <= 1 {
			return max_mip_levels;
		}
		if coverage >= 0.1 {
			return max_mip_levels.saturating_sub(2).max(1);
		}
					...
					.map(|y| {
						...
						for x in 0..next_w {
							...
							let sample = |px: u32, py: u32| -> [u32; 4] {
								...
								[
									prev_pixels[i] as u32,
									prev_pixels[i + 1] as u32,
									prev_pixels[i + 2] as u32,
									prev_pixels[i + 3] as u32,
								]
							};
							let a = sample(sx, sy);
							let b = sample(sx1, sy);
							let c = sample(sx, sy1);
							let d = sample(sx1, sy1);
							let di = (x * 4) as usize;
							for ch in 0..4 {
...
/// loaded audio asset.
///
/// game code references sounds via [`Handle<Sound>`] and passes handles to the
/// audio system to play effects or music.
pub struct Sound {
