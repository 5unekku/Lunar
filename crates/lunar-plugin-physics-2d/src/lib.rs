...
/// plugin that registers velocity integration and collision response.
///
/// register after [`Plugin2d`] — requires `CollisionWorld` to be populated first.
...
pub fn resolve_collisions(
	...
	mut query: Query<
		(Entity, &mut LocalTransform, &mut Velocity2d, &Collider),
		Without<OneWayPlatform>,
	>,
	...
	for (entity, mut transform, mut velocity, collider) in &mut query {
		let ColliderShape::Aabb { half_extents } = collider.shape else {
			continue;
		};
		...
		for other_entity in collision_world.overlapping(entity) {
			...
			let Ok((other_collider, other_transform)) = collider_query.get(other_entity) else {
				continue;
			};
			let ColliderShape::Aabb {
