...
impl WindowHost {
    ...
    #[must_use]
    pub fn new(window: sdl3::video::Window, mouse: sdl3::mouse::MouseUtil, width: u32, height: u32) -> Self {
        Self {
            window,
            mouse,
            fullscreen: false,
    ...
    pub fn sync(&mut self, world: &mut World, mut resize: impl FnMut(&mut World, u32, u32)) {
            ...
            && settings.cursor_locked != self.cursor_locked
        {
            ...
            self.mouse.set_relative_mouse_mode(&self.window, self.cursor_locked);
        }
        ...
        if w != self.last_w || h != self.last_h {
            let target = world.get_resource::<WindowSettings>()
                .and_then(|s| if !self.fullscreen { s.target_aspect } else { None });
