...
//! the built-in engine only ever calls [`AudioSource::fill`] and [`AudioSource::is_done`]: everything else
//! is encapsulated in the implementor.

/// produces f32 interleaved stereo samples at [`SAMPLE_RATE`] Hz.
///
/// the audio callback invokes [`AudioSource::fill`] on each active source once per buffer.
/// sources are dropped automatically when [`AudioSource::is_done`] returns `true`.
