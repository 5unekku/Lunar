...
fn main() {
    ...
    let quality_str = args.windows(2)
        .find(|w| w[0] == "--quality")
        .map(|w| w[1].as_str())
        .unwrap_or("normal");
    ...
    let paths: Vec<PathBuf> = glob::glob(&pattern).unwrap()
        .filter_map(|e| e.ok())
        .collect();
    ...
    let results: Vec<(&PathBuf, Result<(), String>)> = paths.par_iter()
...
fn compress_one(path: &Path, quality: Quality) -> Result<(), String> {
    ...
    let is_normal = name.ends_with("_n") || name.ends_with("_normal")
        || name.ends_with("_nrm") || name.ends_with("normals");
    ...
    let format = if is_normal {
        ...
        if has_alpha { ImageFormat::BC7RgbaUnorm } else { ImageFormat::BC1RgbaUnorm }
    };

    let surface = SurfaceRgba8::from_image(&rgba);
    let encoded = surface.encode(format, quality, Mipmaps::GeneratedAutomatic)
