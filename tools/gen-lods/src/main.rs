...
fn write_mesh(path: &Path, vertex_bytes: &[u8], indices: &[u32]) {
	...
	let mut out =
		Vec::with_capacity(8 + vertex_bytes.len() + index_count * if use_u16 { 2 } else { 4 });
...
fn process_mesh(path: &Path, thresholds: &[f32; 5]) {
	...
	for (level, &ratio) in RATIOS.iter().enumerate() {
		...
		println!(
			"  lod{}: {} verts, {} tris ({:.0}% of source)",
			level + 1,
			lod_vcount,
			lod_tcount,
			lod_tcount as f32 / (source.indices.len() / 3) as f32 * 100.0
		);
	...
	for (i, file) in lod_files.iter().enumerate() {
		...
		let dist_str = if dist == f32::INFINITY {
			"null".to_string()
		} else {
			format!("{dist:.1}")
		};
		ron.push_str(&format!(
			"        ( file: \"{file}\", max_dist: {dist_str} ),\n"
		));
	}
	ron.push_str("    ],\n)\n");

	std::fs::write(&descriptor_path, ron)
		.unwrap_or_else(|e| panic!("write {}: {e}", descriptor_path.display()));
...
fn main() {
	...
	while i < args.len() {
		if args[i] == "--thresholds" {
			...
			let parts: Vec<f32> = args[i]
				.split(',')
				.map(|s| s.parse().expect("threshold must be a float"))
				.collect();
			assert!(
				parts.len() == 5,
				"--thresholds requires 5 comma-separated values (lod0..lod4)"
			);
	...
	paths
		.par_iter()
		.for_each(|path| process_mesh(path, &thresholds));
}
